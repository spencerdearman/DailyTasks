...
/// Distinguishes deliberate pulls (slow, gradual) from scroll-bounce (fast spike).
struct PullToAgent: ViewModifier {
    ...
    @State private var growthFrames: Int = 0
    @State private var isDeliberatePull = false

    private let threshold: CGFloat = 70
    // Require the offset to grow for at least this many frames to count as deliberate
    private let minGrowthFrames = 4

    private var progress: CGFloat {
        guard pullOffset > 0, isDeliberatePull else { return 0 }
        ...
        GeometryReader { _ in
            content
                .overlay(alignment: .top) {
                    if pullOffset > 15 && isDeliberatePull {
                        pullIndicator
                            .frame(height: 50)
                            .offset(y: min(pullOffset * 0.35, 45) - 45)
                            .opacity(min(pullOffset / 35, 1.0))
    ...
    private func handlePullChange(old: CGFloat, new: CGFloat) {
        // Track consecutive frames where offset is growing (user actively pulling)
        if new > old && new > 0 {
            growthFrames += 1
        } else if new <= 0 {
            growthFrames = 0
            isDeliberatePull = false
        }

        // A deliberate pull grows gradually over multiple frames.
        // A bounce spikes past the threshold in 1-2 frames then immediately drops.
        if growthFrames >= minGrowthFrames && !isDeliberatePull {
            isDeliberatePull = true
        }

        guard isDeliberatePull else { return }
        ...
        if new < threshold * 0.5 {
            hitThreshold = false
        }

        // Release detection: offset is dropping and we were past threshold
        let isReleasing = old > new && old >= threshold

        if !triggered && isReleasing {
            triggered = true
            UIImpactFeedbackGenerator(style: .rigid).impactOccurred()
            overlayMode.wrappedValue = .agent
        }

        // Reset when fully settled
        if new < 2 {
            growthFrames = 0
            isDeliberatePull = false
            triggered = false
