...
struct TimelineProvider: AppIntentTimelineProvider {
    ...
    func placeholder(in context: Context) -> TaskEntry {
        TaskEntry(
            date: Date(),
            configuration: TetherIntent(),
            taskData: SharedTaskItem(completedCount: 2, totalCount: 5)
        )
    }

    func snapshot(for configuration: TetherIntent, in context: Context) async -> TaskEntry {
        TaskEntry(date: Date(), configuration: configuration, taskData: fetchSharedData())
    }

    func timeline(for configuration: TetherIntent, in context: Context) async -> Timeline<TaskEntry> {
    ...
    func recommendations() -> [AppIntentRecommendation<TetherIntent>] {
        return [AppIntentRecommendation(intent: TetherIntent(), description: "Default")]
    }
