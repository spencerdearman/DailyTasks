...
final class GoogleCalendarService {
    ...
    /// Presents the Google Sign-In flow in the given window.
    ...
    func restorePreviousSignIn() async -> Bool {
        do {
            ...
            // Ensure calendar scope is granted
    ...
    func events(from startDate: Date, to endDate: Date) async throws -> [CalendarEvent] {
        guard let user = GIDSignIn.sharedInstance.currentUser else {
            print("[GoogleCal] No current user")
            return []
        }
        ...
        guard let accessToken = user.accessToken.tokenString as String? else {
            print("[GoogleCal] No access token")
            return []
        }

        let calendarIDs = try await fetchCalendarIDs(accessToken: accessToken)
        print("[GoogleCal] Found \(calendarIDs.count) calendars: \(calendarIDs)")
        ...
        for calendarID in calendarIDs {
            ...
            guard let (data, response) = result,
                ...
                print("[GoogleCal] No response for calendar: \(calendarID)")
                continue
            }

            if http.statusCode != 200 {
                let body = String(data: data, encoding: .utf8) ?? "?"
                print("[GoogleCal] Error \(http.statusCode) for \(calendarID): \(body.prefix(200))")
                continue
            }

            let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
            if let items = json?["items"] as? [[String: Any]] {
                let parsed = items.compactMap { parseEvent($0) }
                print("[GoogleCal] \(calendarID): \(items.count) raw, \(parsed.count) parsed")
                allEvents += parsed
            }
        }

        print("[GoogleCal] Total events: \(allEvents.count)")
        return allEvents
    }
    ...
    private func fetchCalendarIDs(accessToken: String) async throws -> [String] {
        ...
        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            print("[GoogleCal] calendarList failed: \((response as? HTTPURLResponse)?.statusCode ?? -1)")
            return ["primary"]
        }
