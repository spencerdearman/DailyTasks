...
struct DailyPlanCard: View {
    ...
    var weatherService: TetherWeatherService? = nil
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .trailing, spacing: 1) {
                    ...
                    if let weather = resolvedWeather {
                        Text(weather)
    ...
    /// Infer the target date from event cards or message content, falling back to today.
    private var targetDate: Date {
        // If we have event cards, use the earliest event's date
        if let events = eventCards, let first = events.first {
            return first.startDate
        }
        // Check message text for "tomorrow"
    ...
    private var headerTitle: String {
        ...
        if cal.isDateInToday(targetDate) {
            return "Your Day"
        } else if cal.isDateInTomorrow(targetDate) {
            return "Tomorrow"
        } else {
            let fmt = DateFormatter()
            fmt.dateFormat = "EEEE"
            return "\(fmt.string(from: targetDate)) Plan"
        }
    }
    ...
    private var resolvedWeather: String? {
        // Try date-specific forecast from weather service first
        if let service = weatherService {
            return service.summary(for: targetDate)
        }
        // Fall back to the passed-in summary (today only)
