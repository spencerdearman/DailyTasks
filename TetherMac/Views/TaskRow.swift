...
struct TaskRow: View {
    ...
    private var hasCompactMeta: Bool {
        ...
        || (task.locationName != nil && !(task.locationName?.isEmpty ?? true))
    }
            ...
            HStack(spacing: 0) {
                ...
                HStack(spacing: 2) {
                    ...
                    Button {
                        ...
                        let hasLocation = task.locationName != nil && !(task.locationName?.isEmpty ?? true)
                        Image(systemName: hasLocation ? "location.fill" : "location")
                            .font(.system(size: 14))
                            .foregroundStyle(showLocationPopover ? Color.primary : (hasLocation ? Color.red : Color.secondary))
                            .frame(width: 30, height: 28)
